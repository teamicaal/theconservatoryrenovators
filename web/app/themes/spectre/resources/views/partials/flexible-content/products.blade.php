                ...
                @endphp
                  <div class="lg:w-1/3 sm:w-1/2 w-full sm:px-4 mb-8 mix {{ strtolower(str_replace(',-', ' ', str_replace(' ', '-', $title))) }}">
                    <a class="group" href="#contact-us">
                      <div class="relative">
                        <div class="embed embed-4by3 z-10">
                          <img data-src="{{ $image['sizes']['4by3-md'] }}" alt="{{ $image['alt'] }}" class="lozad object-cover-absolute" />
                        </div>
                        <div class="w-11/12 mx-auto mt-[-2.5rem] p-5 bg-gray-100 flex justify-between items-center rounded-sm relative z-20 transition group-hover:bg-primary">
