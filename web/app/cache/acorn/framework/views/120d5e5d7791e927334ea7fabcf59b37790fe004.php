    ...
    <div class="w-full lg:w-1/2 xl:w-2/5 p-8 relative flex items-center">
      <div class="w-full lg:h-full flex flex-col justify-between rounded-xl bg-white">
        <div class="mb-8 lg:mb-0 p-8">
          <a href="<?php echo e(get_bloginfo('url')); ?>" class="border-none">
            <?php if( $icon ): ?>
              <img src="<?php echo $icon['url']; ?>" alt="<?php echo get_bloginfo('name', 'display'); ?> Logo"<?php echo $icon_retina ? ' srcset="' . $icon['url'] . ' 1x, ' . $icon_retina['url'] . ' 2x"' : null; ?> width="48" height="48" class="opacity-100 transition hover:opacity-75">
            <?php elseif( $logo ): ?>
              <img src="<?php echo $logo['url']; ?>" alt="<?php echo get_bloginfo('name', 'display'); ?> Logo"<?php echo $logo_retina ? ' srcset="' . $logo['url'] . ' 1x, ' . $logo_retina['url'] . ' 2x"' : null; ?> width="auto" height="48" class="opacity-100 transition hover:opacity-75">
            <?php else: ?>
              <h1 class="text-primary font-bold text-xl"><?php echo e(get_bloginfo('name', 'display')); ?></h1>
            <?php endif; ?>
          </a>
        </div>
        <div class="w-full px-8 lg:px-16 xl:px-32 mb-4 lg:mb-0">
          <?php if( $login === 'failed' ): ?>
            <?php echo '<div class="alert alert-danger text-xs">The login details you entered are incorrect. Please try again.</div>'; ?>

          <?php elseif( $login === 'empty' ): ?>
            <?php echo '<div class="alert alert-danger text-xs">Please enter both your username and password to login.</div>'; ?>

          <?php elseif( $login === 'false' ): ?>
            <?php echo '<div class="alert alert-info text-xs">You are now logged out.</div>'; ?>

          <?php elseif( $login === 'media' ): ?>
            <?php echo '<div class="alert alert-info text-xs">Cant login? <a href="/contact/" target="_blank">Request access to the media centre.</a></div>'; ?>

          <?php elseif( is_user_logged_in() ): ?>
            <?php echo '<div class="alert alert-info text-xs">You are already logged in. <a href="' . wp_logout_url() . '">Log out?</a></div>'; ?>
...
<?php echo $__env->make('layouts.login', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Users/mattpickles/Local Sites/wraith/web/app/themes/wraith/resources/views/template-login.blade.php ENDPATH**/ ?>